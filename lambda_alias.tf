...
resource "null_resource" "clean_old_versions" {
  ...
  triggers = {
    etag = try(var.alias_config.force_delete_old_versions == true ? timestamp() : data.archive_file.archive.output_base64sha256, data.archive_file.archive.output_base64sha256)
  }

  provisioner "local-exec" {
    working_dir = path.module
    command     = "bash clean_old_lambda_versions.sh ${var.config.function_name} ${var.alias_config.versions_to_keep}"
