...
exports.handler = async () => {
  try {
    ...
    do {
      ...
      all_functions.push(
        ...Functions.filter(
          (x) => x.FunctionName === FunctionName && x.Version != "$LATEST"
        )
      );
    } while (Marker != null);
    ...
    let lambdas = all_functions.sort((a, b) => {
      return a.Version.localeCompare(b.Version, undefined, {
        numeric: true,
      });
    });
    for (let index = 0; index < lambdas.length - versions_to_keep; index++) {
      await lambda
        .deleteFunction({
          FunctionName,
        ...
        .promise();
      console.log(`${FunctionName}:${lambdas[index].Version} deleted`);
    }
  } catch (error) {
    console.error(error);
  }
